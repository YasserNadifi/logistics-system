...
// Form used for editing inventory (only quantity & reorderThreshold are editable)
const InventoryEditForm = ({ inventory, setInventory, onSubmit, onCancel }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <input
          type="text"
          value={inventory.productDto ? inventory.productDto.name : ''}
          readOnly
          className="w-full px-3 py-2 border border-gray-100 rounded-lg bg-gray-50"
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            onChange={(e) => setInventory({ ...inventory, quantity: e.target.value === '' ? '' : parseFloat(e.target.value) })}
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setInventory({ ...inventory, reorderThreshold: e.target.value === '' ? '' : parseFloat(e.target.value) })}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            placeholder="0.0"
          />
        </div>
...
// Modal showing inventory + nested product details (read-only)
const InventoryDetails = ({ inventory }) => {
  ...
  const product = inventory.productDto || {};
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-6">
        ...
        <div>
          <h4 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-2">Product</h4>
          <div className="space-y-3">
            <div>
            ...
            <div>
              ...
              <p className="font-medium text-gray-900 flex items-center">
                ...
                {product.productionDurationMinutes ? `${product.productionDurationMinutes} min` : '-'}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-500">Created At</p>
              <p className="font-medium text-gray-900">{formatDateTime(product.createdAt)}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">Last Updated</p>
              <p className="font-medium text-gray-900">{formatDateTime(product.updatedAt)}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div>
        <h4 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-2">Product Description</h4>
...
const ProductInventory = ({ userRole = 'ADMIN' }) => {
  ...
  useEffect(() => {
    const fetchInventories = async () => {
      try {
        const res = await axios.get(`${API_URL}/product-inventory`, {
  ...
  const filtered = inventories
    .filter(inv => {
      ...
      const productName = inv.productDto?.name ?? '';
      return (
        (productName && productName.toLowerCase().includes(q)) ||
        (inv.id && inv.id.toString().includes(q)) ||
        (inv.productDto?.id && inv.productDto.id.toString().includes(q))
      );
    })
    .sort((a, b) => {
  ...
  const handleSave = async () => {
    try {
      // Ensure numeric values are numbers (backend expects numbers)
      const payload = {
        ...selectedInventory,
        quantity: selectedInventory.quantity === '' ? 0.0 : Number(selectedInventory.quantity),
        reorderThreshold:
          selectedInventory.reorderThreshold === '' ? 0.0 : Number(selectedInventory.reorderThreshold)
      };

      // Assuming backend accepts PUT /inventory with the full object
      await axios.put(`${API_URL}/product-inventory`, payload, {
        headers: { Authorization: `Bearer ${token}` }
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">View and update inventory quantities and reorder thresholds</p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search by product name, inventory ID or product ID..."
            value={searchTerm}
            onChange={(e) => {
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th
                  className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                  onClick={() => handleSort('productDto.name')}
                >
                  Product Name {sortField === 'productDto.name' && (sortDirection === 'asc' ? '↑' : '↓')}
                </th>
                <th
                  className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                ...
                paginated.map((inv) => (
                  <tr key={inv.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">{inv.productDto?.name}</div>
                      {inv.productDto?.sku && (
      ...
      {/* Edit modal */}
      <Modal isOpen={showEditModal} onClose={() => setShowEditModal(false)} title="Edit Inventory" size="md">
        <InventoryEditForm
      ...
      <Modal isOpen={showViewModal} onClose={() => setShowViewModal(false)} title="Inventory Details" size="xl">
        {selectedInventory && <InventoryDetails inventory={selectedInventory} />}
      </Modal>
    </div>
  );
};
